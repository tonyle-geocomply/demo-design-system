...
const GcDropdown = class {
  ...
  render() {
    return (h(Host, null, h("div", { onClick: () => this.handleClick(), onMouseEnter: () => this.handleHover(), onMouseLeave: () => this.handleHover(), class: "slot-container", id: "host-element", "aria-describedby": "tooltip", ref: elm => (this.containerElm = elm) }, h("slot", null), this.suffixArrow ? h("span", { class: "suffix-arrow" }, h("gc-icon", { size: "12px", name: `fa-solid fa-caret-${this.isToggle ? 'up' : 'down'}` })) : null), h("div", { onClick: () => this.handleClickDropdown(), class: "gc__dropdown-content", id: "tooltip", role: "tooltip", ref: elm => (this.dropdownElm = elm) }, this.renderItems(), h("slot", { name: "gc__dropdown-content" }), h("div", { id: "arrow", "data-popper-arrow": true }))));
  }
...
const GcFormField = class {
  ...
  renderField() {
    switch (this.type) {
      ...
      default:
        return (h("gc-input", { "prefix-icon": this.prefixIcon, "gc-id": this.gcId, "gc-name": this.gcName, value: this.value, disabled: this.disabled, type: this.type, placeholder: this.placeholder, "onGc:change": e => this.handleChange(e), "is-error": !!this.errorText || this.isError }));
    }
  }
  render() {
...
const GcTable = class {
  ...
  renderBodyWithExpandableRows() {
    ...
    treeData.forEach(expandedRow => {
      ...
      const expandableRows = (h("gc-cell-expandable", { class: { 'is-loading': this.loadingGroupIndex.includes(`${index}`) }, index: index, fieldName: fieldName, value: value, total: total, totalText: totalText, linkTo: linkTo, tooltipMessage: tooltipMessage, numberOfEntryPerPage: numberOfEntryPerPage || data.length, maxWidth: this.maxWidthInExpandRow, isLoading: this.loadingGroupIndex.includes(`${index}`), width: this.width }, this.loadingGroupIndex.includes(`${index}`) && h("div", { class: "loading-section" }, rows.length >= 4 ? h("gc-spinner", null) : null), rows));
...
const GcTextarea = class {
  ...
  render() {
    return (h(Host, null, h("textarea", { style: { height: this.height, resize: this.resize }, class: `${this.class || ''} ${this.isError ? 'has-error' : ''}`, name: this.gcName, onInput: this.onInput, id: this.gcId, value: this.value, placeholder: this.placeholder, disabled: this.disabled, maxlength: this.maxlength, rows: this.rows, cols: this.cols })));
  }
};
...
const GcUpload = class {
  ...
  render() {
    if (this.errorState) {
      return (h(Host, null, h("form", { id: "dropzone", class: { 'dropzone': true, 'dropzone-dragging': this.dragging }, ref: el => (this.container = el), style: { width: this.width } }, h("div", { class: "dz-message" }, h("div", { class: "gc__dropzone-icon" }, this.errorState === 'type-error' ? (h("gc-icon", { customStyle: { '--fa-primary-color': 'var(--gc-color-red)', 'fontSize': '60px', '--fa-secondary-color': '#D0D8E0' }, name: "fa-duotone fa-circle-exclamation" })) : (h("gc-icon", { customStyle: { '--fa-primary-color': 'var(--gc-color-red)', 'fontSize': '60px', '--fa-secondary-color': '#D0D8E0' }, name: "fa-duotone fa-file-circle-xmark" }))), h("div", { class: "gc__dropzone-body gc__dropzone-body--error" }, h("div", { class: "error-text" }, this.errorState === 'type-error' ? 'Could not load your file, the format is invalid.' : 'Your file too large and can not be uploaded!'), h("div", null, this.errorState === 'type-error' ? `Please try again with *.${this.acceptType} file format` : 'Please reduce the file size and try again!')), h("div", { class: "gc__dropzone-buttons" }, h("gc-button", { id: "browse_files", type: "primary", "padding-text": "30px", height: "32px" }, "Browse Files"))))));
    }
    if (this.isCustom) {
    ...
    return (h(Host, null, h("form", { id: "dropzone",
      // action="/upload"
      class: { 'dropzone': true, 'dropzone-dragging': this.dragging }, ref: el => (this.container = el), style: { width: this.width } }, h("div", { class: "dz-message" }, !this.fileName && (h("div", { class: "gc__dropzone-heading" }, h("slot", { name: "gc__dropzone-heading" }))), h("div", { class: "gc__dropzone-icon" }, h("gc-icon", { name: `fa-regular ${this.getIcon()}`, size: "40px", color: "var(--gc-color-primary)" }), h("div", { class: "gc__dropzone-extension" }, "*.", this.acceptType)), !!this.fileName && h("div", { class: "gc__dropzone-filename" }, this.fileName), !this.fileName && (h("div", { class: "gc__dropzone-body" }, h("slot", { name: "gc__dropzone-body" }))), !this.fileName && (h("div", { class: "gc__dropzone-buttons" }, h("gc-button", { id: "browse_files", type: "primary", "padding-text": "30px", height: "32px" }, "Browse Files"), h("div", { class: "gc__dropzone-type" }, "Drop your *.", this.acceptType, " file here"))), !this.fileName && (h("div", { class: "gc__dropzone-notes" }, h("slot", { name: "gc__dropzone-notes" }))))), !!this.progress && h("gc-progress", { percent: this.progress, width: `calc(${this.width} + 45px)` })));
  }
};
GcUpload.style = gcUploadCss;

export { GcAlert as gc_alert, GcButton as gc_button, GcCard as gc_card, GcCellExpandable as gc_cell_expandable, GcCheckbox as gc_checkbox, GcDragContainer as gc_drag_container, GcDraggableItem as gc_draggable_item, GcDropdown as gc_dropdown, GcFormField as gc_form_field, GcH1 as gc_h1, GcH2 as gc_h2, GcH3 as gc_h3, GcIcon as gc_icon, GcInput as gc_input, GcLabel as gc_label, GcLink as gc_link, GcMenu as gc_menu, GcMenuItem as gc_menu_item, GcModal as gc_modal, GcOl as gc_ol, GcPagination as gc_pagination, GcProgress as gc_progress, GcSelect as gc_select, GcSpinner as gc_spinner, GcStep as gc_step, GcSteps as gc_steps, GcTab as gc_tab, GcTabPanel as gc_tab_panel, GcTable as gc_table, GcTabs as gc_tabs, GcTabsList as gc_tabs_list, GcTag as gc_tag, GcTextarea as gc_textarea, GcTooltip as gc_tooltip, GcUl as gc_ul, GcUpload as gc_upload };
