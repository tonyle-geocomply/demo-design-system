...
const GcLink$1 = /*@__PURE__*/ proxyCustomElement(class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
    this.__attachShadow();
  ...
  render() {
    if (this.icon) {
      return (h("div", { style: { display: 'flex', alignItems: 'center' } }, h("gc-icon", { name: this.icon, size: "sm", color: "#397FF7" }), h("a", { style: { color: this.color || 'var(--gc-color-primary)', marginLeft: '8px' }, class: this.getClassName(), id: this.gcId, href: this.gcTo }, h("slot", null))));
    }
    return (h("a", { style: { color: this.color || 'var(--gc-color-primary)' }, class: this.getClassName(), id: this.gcId, href: this.gcTo }, h("slot", null)));
  }
  ...
  components.forEach(tagName => { switch (tagName) {
    case "gc-link":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, GcLink$1);
      }
      break;
    case "gc-icon":
      if (!customElements.get(tagName)) {
...
const GcLink = GcLink$1;
const defineCustomElement = defineCustomElement$1;

export { GcLink, defineCustomElement };
