...
const GcFormField = class {
  ...
  renderField() {
    switch (this.type) {
      case 'select':
        return (index$1.h("gc-select", { search: this.search, items: this.items, "gc-id": this.gcId, "gc-name": this.gcName, value: this.value, disabled: this.disabled, placeholder: this.placeholder, "onGc:change": e => this.handleChange(e), "is-error": !!this.errorText }));
      case 'textarea':
        return (index$1.h("gc-textarea", { "gc-id": this.gcId, "gc-name": this.gcName, value: this.value, disabled: this.disabled, placeholder: this.placeholder, "onGc:change": e => this.handleChange(e), "is-error": !!this.errorText, cols: this.cols, rows: this.rows, height: this.height, maxlength: this.maxlength }));
      default:
...
const GcInput = class {
  ...
  render() {
    return (index$1.h(index$1.Host, null, index$1.h("input", { class: this.prefixIcon ? `has-prefix ${this.class || ''} ${this.isError ? 'has-error' : ''}` : `${this.class || ''} ${this.isError ? 'has-error' : ''}`, name: this.gcName, onInput: this.onInput, id: this.gcId, type: this.type, value: this.value, placeholder: this.placeholder, disabled: this.disabled }), this.prefixIcon && index$1.h("gc-icon", { color: "var(--gc-color-primary)", name: this.prefixIcon })));
  }
};
...
const GcTable = class {
    ...
    columnsWithPos.forEach((col, i) => {
      if (this.showingColumns[col.name]) {
        ...
        const colEl = (index$1.h("div", { onClick: () => {
          ...
          }, class: { gc__col: true, sort: this.sortBy === col.name }, style: { width: colWidth, background: this.background, padding: col.padding } }, index$1.h("div", { class: "col-content" }, index$1.h("div", { class: "col-text" }, col.label), index$1.h("div", { class: "col-actions" }, (() => {
        ...
        if (this.showingColumns[column.name]) {
          ...
          if (i === columnsWithPos.length - 1 && column.name === 'custom_actions') {
            fixedLastCol = (index$1.h("div", { class: { 'gc__col': true, 'col-hover': this.hoveredCell.row === row && this.hoveredCell.column === column, 'col-center': column.center }, style: {
                ...
                if (selection.type != 'Range')
                  this.onCellClick(row, column);
              } }, index$1.h("div", { class: "col-content", style: { justifyContent: 'space-evenly' } }, column.actions.map(action => (index$1.h("gc-button", { "onGc:click": () => action.onClick(row), type: action.type, height: "27px", paddingText: "8px" }, index$1.h("gc-icon", { size: "16px", name: action.icon })))))));
          }
          else {
  ...
  renderEmptyState() {
    if (this.customEmptyState) {
      return index$1.h("div", { innerHTML: this.customEmptyState });
    }
...
const GcTooltip = class {
  ...
  render() {
    return (index$1.h("div", { onClick: () => this.onToggleTooltip(), style: { color: 'var(--gc-color-text-grey)', textDecoration: 'underline', cursor: 'pointer' }, class: { 'has-tooltip': true, 'active': this.isToggle ? this.showTooltip && this.isToggle : this.showTooltip } }, this.renderCutText(), index$1.h("span", { class: "tooltip-wrapper" }, index$1.h("div", { class: "tooltip", style: { right: this.rightPos || '35%', top: this.topPos || '1rem' } }, this.content, this.getIsCopyText() && (index$1.h("div", { style: { marginTop: '8px' } }, index$1.h("gc-button", { height: "29px", type: "primary", "onGc:click": () => copyClipboard(this.content, () => {
