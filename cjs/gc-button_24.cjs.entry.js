...
const GcLink = class {
  ...
  render() {
    if (this.icon) {
      return (index$1.h("div", { style: { display: 'flex', alignItems: 'baseline', cursor: 'pointer' } }, index$1.h("gc-icon", { onClick: () => this.onClickIcon(), name: this.icon, size: "13px", color: "#397FF7" }), index$1.h("a", { target: this.target, style: { color: this.color || 'var(--gc-color-primary)', marginLeft: '8px' }, class: this.getClassName(), id: this.gcId, href: this.gcTo }, index$1.h("slot", null))));
    }
...
const GcTable = class {
  ...
  renderColumnContent(row, column, conditionToDisplayActions) {
    if (column.isLongText || column.isCopyText) {
      return (index$1.h("gc-dropdown", null, index$1.h("div", { style: { color: 'var(--gc-color-text-grey)', textDecoration: 'underline', cursor: 'pointer' }, class: "col-text" }, this.renderCutText(row, column), this.renderActions(row, column, conditionToDisplayActions)), index$1.h("div", { slot: "gc__dropdown-content", class: "menu", style: { width: '300px', padding: '10px' } }, row === null || row === void 0 ? void 0 :
        row[column.name], column.isCopyText && (index$1.h("div", { style: { marginTop: '8px' } }, index$1.h("gc-button", { height: "29px", type: "primary", "onGc:click": () => copyClipboard(row === null || row === void 0 ? void 0 : row[column.name]) }, column.isCopyText.text || 'Copy'))))));
    }
  ...
  renderBody() {
    ...
    data.forEach((row, idx) => {
      ...
      rows.push(index$1.h("div", { class: { 'gc__row': true, 'row-hover': this.hoveredCell.row === row }, style: {
        ...
        } }, index$1.h("div", { class: "fixed-columns columns-container", style: { position: countCurrentCol.length <= DEFAULT_MAXIMUM_TO_SCALE ? 'relative' : 'sticky' } }, fixedCols), index$1.h("div", { class: "scrollable-columns columns-container" }, scrollCols)));
    });
